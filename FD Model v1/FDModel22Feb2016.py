    ...
    for key in regProdsA: #these are the actual lifetimes of the outgoing Prods
        ...
        if (regProdsA[key] == 0):
            ...
            #additional empty bin available
                eBinInUse -= 1
            # we still need to track this product, until the incoming has cleared
            # garbOut.add(key)
        elif ((regProdsA[key] < 0) and (key not in garbOut)): 
            print "Warning1! regProdsA[",key,"] has run out, is", regProdsA[key]
