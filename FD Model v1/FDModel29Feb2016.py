...
eTime = 8 #this is the arbitrary expected time for regular product arrival
delta = 3 #this is the arbitrary variance for reg prod arrival

arrTime = 6 #arbitrary arrival time for new sProd candidate
shelfTime = 10 #arbitrary OOS time for new sProd candidate
...
def dispBinState():
    print "################################"
    print "binNotInUse is", binNotInUse
    print "numTotal is", numTotal
    print "hPrdInUse is", hPrdInUse
    print "sPrdInUse is", sPrdInUse
    print "eBinInUse is", eBinInUse
    print "################################"
...
def step():
    ...
    print "we have", SHVel, "candidate(s) in the SHVel pool this step"
    ...
    print "we were able to bin", SHVelSuccess, "SHVel Prod(s), add", SHVelPoints,"points to stepScore for each, +", SHVelSuccess*SHVelPoints, "total points"
    stepScore -= SHVel * SHVelPoints #the remaining SHVel is the number of SHVel not binned
    print "we were not able to bin", SHVel, "SHVel Prod(s), minus", SHVelPoints,"points to stepScore for each, -", SHVel*SHVelPoints, "total points"
    ...
    print "we have", HVel, "candidate(s) in the HVel pool this step"
    ...
    print "we were able to bin", HVelSuccess, "HVel Prod(s), add", HVelPoints,"points to stepScore for each, +", HVelSuccess*HVelPoints, "total points"
    # print "we were able to bin a HVel Prod, add", HVelPoints,"points to stepScore for", HVelSuccess, "HV products"
    stepScore -= HVel * HVelPoints #number of HVel not binned
    # print "we were not able to bin a HVel Prod, minus", HVelPoints,"points to stepScore for", HVel, "HV products, total", HVel*HVelPoints
    print "we were not able to bin", HVel, "HVel Prod(s), minus", HVelPoints,"points to stepScore for each, -", HVel*HVelPoints, "total points"
    
    
    
    #penalty for having empty bins not holding anything
    stepScore -= binNotInUse * EmptyBinPenalty
    print "we have", binNotInUse, "bins not in use, a penalty of", EmptyBinPenalty, "for each empty bin, total -", binNotInUse * EmptyBinPenalty
...
def addHVelProds():
    ...
    ### CHANGE THIS
    ################################
    numHi = random.randint(0,6) #arbitrary
    ################################
    ################################
    return numHi
...
def addSHVelProds():
    ...
    ### CHANGE THIS
    ################################
    numSuperHi = random.randint(0,6) #arbitrary
    ################################
    ################################
    return numSuperHi
...
def updateSeasProds(): #TODO
    ...
    for key in sArriveTimes:
        ...
        print "seas key is", key
        print "sArrTimeLeft is", sArrTimeLeft
        print "sStkTimeLeft is", sStkTimeLeft
        ...
        if (sArrTimeLeft > 0): #the seasonal product isn't in yet, no worries
            ...
            if (seasAvail > 0):
                ...
                sPrdInUse += 1
                updateBinState()
                print "Line 319: We now have", sPrdInUse, "seas bin(s) in use."
                stepScore += SeasProdBinPoints
                print "we were able to bin a sProd, add (+)", SeasProdBinPoints,"points to stepScore"
            else:
                #we have no bins available for the sProd
                print "Line 0: there are no more empty bins for sProds"
                print "Line 5: We now have", sPrdInUse, "seasonal bin(s) in use."
                updateBinState()
                stepScore -= SeasProdBinPoints
                print "we were not able to bin a sProd, subtract (-)", SeasProdBinPoints,"points from stepScore"
...
def addSeasProds(): 
    ...
    ### CHANGE THIS
    ################################
    numSeasCand = random.randint(0,2)
    ...
    for i in xrange(numSeasCand):
        ...
        print "sOccTime is", sOccTime
        print "earlyAccomSlot is", earlyAccomSlot
        print "earlyAccomTime is", earlyAccomTime
        ...
        if (arrTime > earlyAccomTime):
            ...
            print "we were able to approve a sProd, add (+)", SeasProdAppPoints,"points to stepScore for 1 sProd"
        else: 
            stepScore -= SeasProdAppPoints
            print "we were not able to approve a sProd, minus (-)", SeasProdAppPoints,"points to stepScore for 1 sProd"
...
def newSeasProd(sProd, arrivalTime, shelflifeTime):
    ...
    currProd += 1
    print "sProd added"
    print sProd, arrivalTime, shelflifeTime
    return
...
def addRegProds():
    ...
    ### CHANGE THIS
    ################################
    numRegProds = random.randint(0,2)
    ################################
    ################################
